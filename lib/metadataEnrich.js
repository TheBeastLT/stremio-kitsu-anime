...
async function enrichKitsuMetadata(metadata, retrieveImdbMetadata) {
  ...
  if (imdbInfo && imdbInfo.imdb_id) {
    ...
    return sanitize({
      ...
      imdbRating: metadata.imdbRating || imdbMetadata.imdbRating,
      genres: metadata.genres.length ? metadata.genres : imdbMetadata.genres,
      videos: await enrichKitsuEpisodes(metadata, imdbMetadata, imdbMetadata),
      links: []
...
function getCinemetaGenres(metadata) {
  return (metadata?.links || [])
      ...
      .map(link => getGenreUrl(link.name));
}
