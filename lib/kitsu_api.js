...
async function search(searchTerm) {
  ...
  return _getCatalogEntries('/api/edge/anime', query)
      .then((entries) => entries.map(entry => {
        ...
        const maxSeason = parsedTitles[distances.indexOf(maxDistance)].season || parser.parse(entry.description).season;
        return { ...entry, sort: { distance: maxDistance, title: maxTitle, season: maxSeason }};
      }))
      .then((entries) => entries.sort((a, b) => {
        const aSeasonSort = a.sort.season === parsedSeason || a.sort.title.endsWith(parsedSeason) ? -1 : 0;
        const bSeasonSort = b.sort.season === parsedSeason || b.sort.title.endsWith(parsedSeason) ? -1 : 0;
        const aTypeSort = !isOvaOrSpecial && isTvShow(a.sort.title, b.sort.title) ? -1 : 0;
        const bTypeSort = !isOvaOrSpecial && isTvShow(b.sort.title, a.sort.title) ? -1 : 0;
