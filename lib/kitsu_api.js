...
async function search(searchTerm) {
  ...
  const compareSeason = (sort) => {
    if (parsedSeason === undefined) return 0;
    if (sort.type !== 'series') return 0;
    const entrySeason = sort.season === undefined ? 1 : sort.season;
    return entrySeason === parsedSeason || sort.title.endsWith(`${parsedSeason}`) ? -1 : 0
  }
  const compareType = (aSort, bSort) => {
    if (isOvaOrSpecial) return 0;
    return isTvShow(aSort.title, bSort.title) ? -1 : 0;
  }
  ...
  return _getCatalogEntries('/api/edge/anime', query)
      .then((entries) => entries.map(entry => {
        ...
        const maxSeason = parsedTitles.map(v => v.season).find(v => v) || parser.parse(entry.description).season;
        return { ...entry, sort: { distance: maxDistance, title: maxTitle, season: maxSeason, type: entry.type }};
      }))
      .then((entries) => entries.sort((a, b) => {
        const aSeasonSort = compareSeason(a.sort);
        const bSeasonSort = compareSeason(b.sort);
        const aTypeSort = compareType(a.sort, b.sort);
        const bTypeSort = compareType(b.sort, a.sort);
        ...
        return distanceSort < 0.2 ? seasonSort || typeSort || distanceSort : distanceSort || seasonSort || typeSort;
      }))
      .then((entries) => entries.map((entry) => {
        delete entry.sort;
        return entry;
...
function initSeasonParser() {
  ...
  parser.addHandler('season', /(?:\W)(\d{1,2})[. ]?(?:st|nd|th)$/i, integer);
