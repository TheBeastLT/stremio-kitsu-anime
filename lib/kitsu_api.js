...
async function list(kitsuIds) {
  const query = {
    'include': 'genres',
    'filter[id]': kitsuIds,
    'page[limit]': PAGE_SIZE
  }
  return _getContent('/api/edge/anime', query)
      .then(response => Promise.all(response.data
          .sort((a, b) => kitsuIds.indexOf(a.id) - kitsuIds.indexOf(b.id))
          .map(result => toStremioCatalogMeta(result, response.included))));
}
...
async function _getCatalogEntries(endpoint, queryParams = {}) {
  ...
  queryParams['filter[status]'] = queryParams['filter[status]'] || 'finished,current,upcoming'; // dont show unreleased and tba
...
async function _getContent(endpoint, queryParams) {
  return axios.get(`${KITSU_URL}${endpoint}`, { params: queryParams, headers: { accept: 'application/vnd.api+json' } })
