...
function getCinemetaMetadata(imdbId) {
  return _getMetadata(imdbId, 'series')
      ...
      .catch(error => {
        console.warn(`failed cinemeta query ${imdbId} due: ${error}`);
...
function _getAddonMetadata(baseUrl, metaId, type, retry = 3) {
  return axios.get(`${baseUrl}/meta/${type}/${metaId}.json`, { timeout: DEFAULT_TIMEOUT })
      .then(response => {
        if (response?.data?.meta?.imdb_id) {
      ...
      .catch(error => {
          if (retry > 0) {
              return _getAddonMetadata(baseUrl, metaId, type, retry - 1);
          }
          throw error;
      })
}
