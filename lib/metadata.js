...
function mergeMetadatas(entries, mainKitsuId) {
  const mainMeta = entries.find((entry) => entry.kitsu_id === mainKitsuId);
  mainMeta.videos = entries
      .filter((entry) => entry.videos && entry.imdbFromSeason > 0 && entry.imdbFromEpisode > 0)
      .sort((a, b) => {
        if (a.imdbFromSeason < b.imdbFromSeason) return -1;
        if (a.imdbFromSeason > b.imdbFromSeason) return 1;
        if (a.imdbFromEpisode < b.imdbFromEpisode) return -1;
        if (a.imdbFromEpisode > b.imdbFromEpisode) return 1;
        return 0;
      })
      .map((entry) => entry.videos
          .map((ep) => ({
            id: ep.id,
            title: ep.title,
            released: new Date( ep.released.getTime() + entry.imdbFromSeason * 1000 + entry.imdbFromEpisode),
            season: entry.imdbFromSeason,
            episode: entry.imdbFromEpisode + ep.episode - 1,
          })))
...
module.exports = { toStremioMeta, mergeMetadatas };