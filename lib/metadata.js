    ...
    if (animeData.episodes.data?.length) {
      ...
      videos = animeData.episodes.data
          .map((ep, index, self) => ({
            ...
            season: 1,
            episode: ep.number,
            thumbnail: ep.thumbnail?.original || fanartImages.thumbnail
          }))
      ...
      videos = [...Array(animeData.episodeCount).keys()]
          ...
          .map((ep) => ({
            ...
            season: 1,
            episode: ep,
            thumbnail: fanartImages.thumbnail
          }))
    }
...
function kitsuLinks(animeData, type) {
  ...
  const genres = (animeData.genres.data || [])
      ...
      .map(genre => ({
        name: genre,
        category: 'Genres',
        url: `stremio:///discover/https%3A%2F%2Fanime-kitsu.strem.fun%2Fmanifest.json/anime/kitsu-anime-popular?genre=${genre}`
      }));
